<?php
header("Location:http://www.asyoulikeitsilvershop.com/silver-flatware/");
exit;
ob_start();
...
$featuredItem=getFeaturedItem($cat);
  ...
  $pageTitle="Silver Flatware, Serving Pieces, Place Settings, Complete Sets | As You Like It Silver Shop, New Orleans, Louisiana";
...
$query=mysql_query("SELECT pattern,brand,item,retail,sale,id,image,description from inventory as a where (category=\"f\" or category=\"ps\" or category=\"fcs\" or category=\"sp\")  and time='n' and display=1 and quantity!=0 and retail>0 order by retail");
...
$f.="<div class=\"row\">	
			...
			<div class=\"cell fourColumns topAlign\">
				<a href=\"flatware.php?cId=$row[id]\" class=\"recentacqs\">
			   ...
			   <div class=\"row\">
			   	...
			   	<div class=\"cell eightColumns\">
			   		<p class=searchrecent>
						<strong>
							<a href=\"flatware.php?cId=$row[id]\">$row[item]</a>
						</strong>
					</p>
			   	</div>
	...
	$statement=($cId)?"SELECT pattern,brand,item,retail,sale,id,image as image1, image2,image3,image$cI as imageMain,`desc$cI` as description,image$nI as nextI,quantity from inventory WHERE id=$cId":"SELECT pattern,brand,item,retail,sale,id,image as image1,image2,image3,`desc` as desc1,desc2,desc3, quantity from inventory where (category=\"f\" OR category=\"sp\" or category=\"fcs\" or category=\"ps\") and image IS NOT NULL and `time`='n' and display=1 and quantity!=0 and retail>0 order by rand() limit 1";
...
if (!$row[image1] || !file_exists("productImages/_BG/$row[image1]")) { 
		$handle="HANDLES/$folder/".str_replace($re,$rw,strtoupper("$row[pattern] $row[brand]")).".jpg";
...
if($imageS2!=''||$imageS3!=''){
	...
	$f.="<div id=\"imageThumbnails\" class=\"row\">
			<div class=\"cell fiveColumns\">
					<a href=\"javascript:changePicture('$imageS1','$imageB1','$desc1')\">
		...
		$f.="<div class=\"cell sixColumns\">
					<a href=\"javascript:changePicture('$imageS2','$imageB2','$desc2')\">
	...
	if($imageS3){
			$f.="<div class=\"cell fiveColumns\">
						<a href=\"javascript:changePicture('$imageS3','$imageB3','$desc3')\">
...
<div class=\"row\">
	<div class=\"cell sixteenColumns centered\">
		<b class=recentadesc>&nbsp;&nbsp;<br> 
			<a rel=\"nofollow\" href=\"".$homePage[$cat]."?showAll=1\" class=recentadesc>View the complete list</A> of As You Like It Silver Shop's featured items in Flatware.
	</div>
</div>
";
 }
