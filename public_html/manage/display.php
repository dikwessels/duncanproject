    ...
    <script language='javascript'>
        ...
        function confirmDelete(id, text) {
            ...
            if (confirm('Delete ' + text)) {
                location = 'display.php?delete=' + id + '&cat=' + top.topframe.cat;
            }
...
<table width=700 cellspacing=0 cellpadding=4 border=1 bordercolor="#A27178">
    <tr>
        <td colspan=6>
            ...
            <a alt="Sort by pattern"
               href='display.php?cat=<? echo "$cat&letter=" . urlencode($letter); ?>&order=pattern,brand,item'>Pattern</a>/
            <a alt="Sort by manufacturer"
               href='display.php?cat=<? echo "$cat&letter=" . urlencode($letter); ?>&order=brand,pattern,item'>Manufacturer</a>

        </th>
        <th><? if ($order == "item,brand,pattern") {
                echo "** ";
            } ?>
            <a href='display.php?cat=<? echo "$cat&letter=" . urlencode($letter); ?>&order=item,brand,pattern'>Item</a>
        </th>
        <th>Image</th>
        <th>Recent</th>
        <th>Gift</th>
