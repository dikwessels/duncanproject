<?php
  ...
  while($row=mysql_fetch_assoc($result)){
   ...
   //$c= "<span style=\"font-size:10pt;font-weight:bold;\">$pattern BY $brand 4 Piece $type Setting:</span>";
    ...
    $query="SELECT quantity,item FROM inventory WHERE (monogram=0 or monogram IS NULL) and quantity>0 AND pattern=\"$pattern\" AND brand=\"$brand\" AND (item=\"TEASPOON\" OR item=\"SALAD FORK\" OR item=\"$type FORK\" OR item=\"$type KNIFE\" $psf ) ORDER BY quantity ASC $limit";
