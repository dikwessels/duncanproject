...
function createFile($folder, $fname,$conn_id,$content,$pattern,$brand,$item){
	...
	global $login_result;
		...
		$f = fopen('temp\temp.html','w');
		
		$origin = ftp_pwd($conn_id);
		
		echo "origin is: $origin<br>";
 ...
 if($item){
 	...
 	$fname = str_replace("/home/asyoulik", "/",$fname);
 
 	echo "file name: $fname<br>";
		$msgSuccess.=" at $fname<BR>";
		$msgFail.=" at $fname<BR>";
		
	if(ftp_chdir($conn_id, $origin)){
		
		echo "changed to origin<br>";
		if(@ftp_chdir($conn_id, "$folder")){
			echo "changed to $folder<br>";	
		}
		else{
			echo "couldn't change to $folder<br>";
		}		
		
	}

		if (ftp_put($conn_id, $fname, 'temp\temp.html', FTP_ASCII)){
			echo $msgSuccess;
			 $i =1;
		}
		else{
			echo "I could not ftp<br>";
			echo $msgFail;
			$i=0;
		}
		return $i;
