...
$query="SELECT * FROM inventory WHERE (quantity IS NOT NULL and quantity!=0) and retail>0 and display=1";
...
if($sgift==1){$where.= " AND gift='y'";} 

if($keywords){$where.=" AND keywords like '%$keywords%'";}

$query.=$where." ORDER BY `time` DESC ";
...
while($row=mysql_fetch_assoc($result)){
	...
	if($image && file_exists("/home/asyoulik/public_html/productImages/_BG/$image")){	
		 $productImage="/productImages/_SM/".substr($image, 0,-4)."_SM.jpg";
	}	
	else{
	    if($handleImage){
