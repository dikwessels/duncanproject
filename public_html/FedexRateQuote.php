...
$request['RequestedShipment']['PackageCount'] = "'".$itemCount."'";
    ...
    if ($response -> HighestSeverity != 'FAILURE' && $response -> HighestSeverity != 'ERROR'){  	
       ...
       // $amount = '<td>$' . number_format($rateReply->RatedShipmentDetails[0]->ShipmentRateDetail->TotalNetCharge->Amount,2,".",",") . '</td>';
        if(array_key_exists('DeliveryTimestamp',$rateReply)){
        	//$deliveryDate=date("m/d/Y",$rateReply->DeliveryTimestamp);
        	$timestamp=$rateReply->DeliveryTimestamp;
        	$deliveryTime=substr($timestamp,0,strpos($timestamp,"T"));
        	$deliveryTime=substr($deliveryTime,5,2)."-".substr($deliveryTime,-2)."-".substr($deliveryTime, 0,4);
        	//$deliveryDate= 'Estimated delivery date: '.$deliveryDate;
        	$deliveryDate= '1 Estimated delivery: ' .$deliveryTime;
        }else if(array_key_exists('TransitTime',$rateReply)){
         
         $deliveryDate=$rateReply->TransitTime;
         $deliveryDate='2 Estimated transit time: '.$deliveryDate;
         //$deliveryDate=convertTransitTimes($deliveryDate);
         
         //$deliveryDate=date("m-d-Y",mktime(0,0,0,date("m"),date("d")+1+$deliveryDate,date("Y")));
         //$deliveryDate='2 Estimated delivery: '. $deliveryDate;
        ...
        $deliveryMessage[$i]=$deliveryDate;
